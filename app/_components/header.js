...
const Header = () => {
  ...
  return (
    <div className='absolute z-50 flex justify-between items-center w-full p-4'>
      ...
      {/* LOGO */}
      <div className={`flex ${isHomePage ? 'justify-center p-6 ' : 'justify-start px-1 -pt-1'} w-full sm:justify-start sm:py-5 sm:px-12 lg:py-7 z-50`}>
        <Link href="/" className='w-[18vw] h-auto xs:w-[14vw] sm:w-[4rem] lg:w-[5rem] rounded-full bg-gradient-to-bl from-black to-[#4A4A4A] sm:border-2 sm:border-slate-600'>
      ...
      <div ref={menuRef}>
        ...
        {/* TEXT */}
        <nav
          className={`absolute top-6 right-8 py-4 flex-col w-full max-w-36 rounded-md shadow-md bg-white/40 backdrop-blur-sm z-50
          sm:flex-row sm:max-w-xs sm:justify-center sm:left-24 sm:py-0 sm:rounded-full sm:border-4 sm:border-slate-600  
          lg:text-xl lg:p-1 lg:max-w-sm lg:top-14   
        ...
        >
          {[
            ...
            <Link
              key={title}
              href={url}
              className={`text-md px-4 p-0.5 font-bold rounded-md text-white hover:text-orange hover:scale-110 
