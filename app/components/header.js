...
const Header = () => {
  ...
  return (
    <div className='topbar fixed flex justify-between items-center w-screen h-12 xs:h-14 sm:h-40'>
      ...
      <Link href="/" className='flex gap-1 items-center w-full p-2 z-20 text-2xl'>
        <div className='w-[12vw] xs:w-[2em] rounded-full sm:mx-16 sm:bg-black sm:border-4 sm:border-slate-400'>
          <Image className="p-[.4em] rotating-element hover:scale-110" src="/logo.png" alt="logo" width={4} height={4} layout="responsive" loading="lazy" />
        </div>
        <h1 className="text-nowrap text-[4.5vw] xs:text-lg sm:hidden drop-shadow-sm">CREATIVE SUPPLY</h1>
      ...
      <div ref={menuRef}>
        ...
        {/* TEXT */}
        <nav
          className={`absolute top-14 right-4 py-2 flex-col w-full max-w-40 rounded-md shadow-md bg-white/40 backdrop-blur-sm 
